...
export default function MovieUpdate() {
    const { apiAuthPut } = useFetch()
    const { user } = useAuth()
    const router = useRouter();
    const { toast } = useToast()
    const { movieID } = useParams<{ movieID: string; }>()
    const { userSelectedMovie, isUserSelectedMovieLoading } = useSelectedMovie(movieID)
    ...
    useEffect(() => {
        async function init() {
            ...
            if (userSelectedMovie?.user && checkUser?.id !== userSelectedMovie.user) {
                router.push("/movie");
                return;
            }

            if (!isUserSelectedMovieLoading) {
                setFormsInput({
                    original_file: undefined,
                    title: userSelectedMovie.title,
                    description: userSelectedMovie.description
                });
            }
        }
    ...
    async function handleMovieSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        if (!hasErrors && original_file) {
            try {
                ...
                const result = await apiAuthPut(`api/movie/${userSelectedMovie.id}`, formData)
                toast({
                    title: "Success!",
                ...
                <form onSubmit={handleMovieSubmit} className="flex flex-col gap-8 w-4/6">
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="text-center font-thin text-lg">
                            Update your favorite film and let others discover what makes it special to you.
                        </div>
                    </div>
                    ...
                    <div className="flex justify-end">
                        <button type="submit" className="text-2xl font-semibold px-6 py-2 rounded-lg bg-[#E50914]" disabled={isSubmitting.form}>
                            {isSubmitting.form ? "Updating" : "Update"}
                        </button>
                    </div>
                </form>
            </div>
